...
Everything in `loom.yaml` **except** `spec.params` definitions:

- `spec.excludes[]`, `spec.includes[]`
- `spec.target.url`, `spec.target.branch`, `spec.target.featureBranch`
- `spec.dynamicParams[].command`
- `spec.modules[].params` values
- All operation fields: `newFiles.source`, `newFiles.dest`, `patch.path`, `patch.target`, `shell.command`, `shell.timeout`, `commitPush.message`, `pr.title`, `pr.body`, etc.
...
| Anything else | Treated as git URL, cloned to temp directory |
...
modules:
  ...
  - name: remote-child
    source: https://github.com/org/module.git  # cloned to temp dir
```
...
Deep-merges a partial YAML document into the target using kustomize's `merge2` library. For full behavioral specification including merge semantics, scalar list handling, and end-to-end examples, see [`pkg/action/SPEC_SMP.md`](../pkg/action/SPEC_SMP.md).

Summary of merge behaviors (B1–B8 in SPEC_SMP.md):
...
```
loom run <moduleDir> [flags]

 1. Parse CLI flags and parameters.
 2. Load loom.yaml from moduleDir.
 ...
 7. For each child module (in declaration order):
    a. Resolve child source (local path or git clone).
    b. Render child params through parent template context.
